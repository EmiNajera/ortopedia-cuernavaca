...
import { Calendar, Clock, ArrowRight, ArrowLeft, Tag as TagIcon } from 'lucide-react';

export default function BlogTagPage({ posts, tag }) {
  const router = useRouter();
  const baseUrl = getSiteUrl();
  const tagUrl = `${baseUrl}/blog/tag/${tag}`;
  const pageTitle = `#${tag} | Blog Ortopedia Cuernavaca`;
  const pageDescription = `Artículos etiquetados con "${tag}" en nuestro blog de ortopedia y rehabilitación. Información especializada y actualizada.`;

  // JSON-LD Structured Data para página de tag
  const tagSchema = {
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="keywords"
          content={`${tag}, ortopedia, rehabilitación, blog, artículos, etiquetas`}
        />
          ...
          <div className="container mx-auto px-6">
            ...
            <nav className="mb-8" aria-label="Breadcrumb">
              <ol className="flex items-center space-x-2 text-sm">
                ...
                <li className="text-gray-400">/</li>
                <li className="text-gray-900 dark:text-white font-medium">#{tag}</li>
              </ol>
            </nav>
...
BlogTagPage.getLayout = (page) => <MarketingLayout>{page}</MarketingLayout>;
...
export async function getStaticPaths() {
  try {
    ...
    posts.forEach((post) => {
      if (post.tags && Array.isArray(post.tags)) {
        post.tags.forEach((tag) => {
          // Normalizar tag: convertir a minúsculas y reemplazar espacios con guiones
    ...
    return {
      paths,
      fallback: 'blocking', // Usar blocking para tags nuevos
    };
  } catch (error) {
...
export async function getStaticProps({ params }) {
  try {
    const { getPostsByTag } = await import('@domains/blog/utils/blogUtils');

    // Buscar posts que tengan este tag (normalizado o no)
    const allPosts = await import('@domains/blog/utils/blogUtils').then((m) => m.getAllPosts());
    const posts = allPosts.filter((post) => {
      if (!post.tags || !Array.isArray(post.tags)) return false;
      return post.tags.some(
        (tag) => tag.toLowerCase().replace(/\s+/g, '-') === params.tag.toLowerCase(),
      );
    });
