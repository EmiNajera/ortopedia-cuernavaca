...
const createRays = (count, cycle, baseIntensity = null) => {
  if (count <= 0) return [];
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    // Use baseIntensity if provided, otherwise use random intensity
    const intensity =
      baseIntensity !== null
        ? baseIntensity * (0.8 + Math.random() * 0.4) // Add some variation around base intensity
        : 0.6 + Math.random() * 0.5;
...
export function LightRays({
  ...
  // Support both 'count' and 'numRays' props for compatibility
  const rayCount = numRays || count;
  const [rays, setRays] = useState([]);
  const cycleDuration = Math.max(speed, 0.1);

  useEffect(() => {
    setRays(createRays(rayCount, cycleDuration, intensity));
  }, [rayCount, cycleDuration, intensity]);
