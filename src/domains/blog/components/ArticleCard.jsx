              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-3">
                    <span
                      className={`px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium border ${getCategoryColor(post.category)}`}
                    >
                ...
                {/* Acciones */}
                {showActions && (
                  <div className="flex sm:flex-col items-center justify-end sm:justify-start space-x-2 sm:space-x-0 sm:space-y-2 sm:ml-4 mt-3 sm:mt-0">
                    <button
                      onClick={handleLike}
                    ...
                    <button
                      onClick={handleShare}
                      className="p-2 text-gray-400 hover:text-blue-500 hover:bg-blue-50 rounded-full transition-colors min-h-[44px] min-w-[44px]"
                    >
      ...
      <Link href={`/blog/${post.slug}`} className="block">
        ...
        <div className="relative h-40 sm:h-48 overflow-hidden">
          ...
          <div className="absolute top-3 sm:top-4 left-3 sm:left-4">
            <span
              className={`px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-semibold border ${getCategoryColor(post.category)}`}
            >
              {categories.find((c) => c.id === post.category)?.icon}{' '}
              {categories.find((c) => c.id === post.category)?.name}
            </span>
        ...
        <div className="p-4 sm:p-6">
          {/* Metadatos */}
          <div className="flex flex-wrap items-center gap-3 sm:gap-4 mb-3 text-xs sm:text-sm text-gray-500">
            <div className="flex items-center">
              <Calendar className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
              {formatDate(post.date)}
            </div>
          ...
          {/* Título */}
          <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-3 line-clamp-2 group-hover:text-blue-600 transition-colors leading-tight">
            {post.title}
          </h3>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center space-x-2">
              <div className="w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-r from-blue-600 to-blue-700 rounded-full flex items-center justify-center text-white text-xs sm:text-sm font-semibold">
                {post.author
                  ? post.author
                      .split(' ')
                      .map((n) => n[0])
              ...
              <span className="text-xs sm:text-sm font-medium text-gray-700">{post.author}</span>
            </div>
